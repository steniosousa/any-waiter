...
function AuthProvider({ children }: AuthProviderProps) {
    ...
    async function signIn(email: string, password: string) {
        setLoading(true);
        try {
            const { data }: { data: DataLogin } = await Api.post("/waiter/auth", { email, password });
            setUser({
                id: data.user.id,
            ...
            await AsyncStorage.setItem('@anygarcom:user', JSON.stringify(data.user));
            await AsyncStorage.setItem('@anygarcom:token', data.token);
        } catch (error) {
